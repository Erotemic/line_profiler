...
def get_column_widths(
    ...
    """
    subconf = ConfigSource.from_config(config).get_subconfig(
        'show', 'column_widths'
    )
        ...
        *,
        ...
        if wrap and members_to_wrap:
            self._wrap_namespace_members(
                namespace, members_to_wrap, warning_stack_level=3
            )
        if count:
            self._debug(
...
def is_generated_code(filename: str) -> bool:
    ...
    return (
        ...
        or filename.startswith(os.path.join(temp_dir, 'ipykernel_'))
        or filename.startswith(os.path.join(temp_dir, 'xpython_'))
    )
    ...
    *,
    ...
    if os.path.exists(filename) or is_generated_code(filename):
        ...
        nlines = (
            1
            if not linenos
            else max(linenos) - min(min(linenos), start_lineno) + 1
        )
        sublines = [''] * nlines
    ...
    lhs_template = ' '.join(
        ['%' + str(column_sizes[k]) + 's' for k in col_order]
    )
    ...
    if rich:
        ...
        write_console = Console(
            file=cast(IO[str], stream), soft_wrap=True, color_system='standard'
        )
...
def show_text(
    ...
    *,
    ...
    if sort:
        ...
        stats_order = sorted(
            stats.items(), key=lambda kv: sum(t[2] for t in kv[1])
        )
    else:
        # Default ordering
...
def main() -> None:
    ...
    add_argument(
        ...
        '--rich',
        action='store_true',
        help=f'Use rich formatting. (Default: {default.conf_dict["rich"]})',
    )
    add_argument(
        parser,
        '-z',
    ...
    add_argument(
        ...
        '--sort',
        action='store_true',
        help=f'Sort by ascending total time. (Default: {default.conf_dict["sort"]})',
    )
    add_argument(
        parser,
        '-m',
